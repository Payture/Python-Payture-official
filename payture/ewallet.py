...
class TransactionEWallet(transaction.Transaction):
    ...
    def expandForMerchantPayReg(self, customer, cardId, secureCode, data):
        ...
        str = customer._getPropertiesString() + '%s=%s;' % (PaytureParams.CardId, cardId) + '%s=%s;' % (
        PaytureParams.SecureCode, secureCode) + data._getPropertiesString() + data.CustomFields;
        return self._expandInternal(constants.PaytureParams.DATA, str)
    ...
    def expandCustomer(self, customer):
        ...
        if (self.Command == constants.PaytureCommands.Delete):
            str += '%s=%s;%s=%s;' % (
            constants.PaytureParams.VWUserLgn, customer.VWUserLgn, constants.PaytureParams.Password,
    ...
    def expandInit(self, customer, cardId, data):
        ...
        self._sessionType = data.SessionType
        str = customer._getPropertiesString() + (
            '' if cardId == None else 'CardId=%s;' % (cardId)) + data._getPropertiesString() + (
    ...
    def expandForCardOperation(self, customer, cardId, amount, orderId=None):
        ...
        if (customer == None or cardId == None):
            return self
        str = customer._getPropertiesString() + '%s=%s;' % (constants.PaytureParams.CardId, cardId) + ('%s=%s;' % (
        constants.PaytureParams.Amount,
        amount) if amount != None and self.Command == constants.PaytureCommands.Activate else '') + (
                  '' if orderId == None else '%s=%s;' % (constants.PaytureParams.OrderId, orderId))
        return self._expandInternal(constants.PaytureParams.DATA, str)
